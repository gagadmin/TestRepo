 ...
 *            audit    -> pages/AuditTrailPage.vue
 *            users    -> pages/UsersPage.vue
...
import { scoreSeverity, severityFor } from '@/composables/useFormatters';
...
/*
 * Tag colours come from the shared map rather than a local copy. The point of
 * `useFormatters` is that one status is one colour on every screen; a private
 * duplicate here is how the two drift apart, which is exactly what had happened
 * to the score thresholds and the invalid-date guard before this file adopted
 * the composable.
 */
const statusSeverity = (status) => severityFor('connectionStatus', status);
...
const slaComplianceSeverity = (compliance) => scoreSeverity(compliance);
...
/*
 * Adopting the shared map changes one colour: a medium-priority ticket tag was
 * grey here and is blue on every other screen. The shared value wins — the
 * inconsistency was the bug.
 */
const prioritySeverity = (priority) => severityFor('ticketPriority', priority, 'secondary');
